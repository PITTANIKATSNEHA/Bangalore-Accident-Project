...
-- view 1. Yearly trends with fatality rate
...
--view 2. Top 10 most dangerous stations
...
-- view 4. Year-over-year comparison
...
-- View 6. Station growth analysis (useful for improvement scoring)
CREATE OR REPLACE VIEW ANALYTICS.v_station_growth_analysis AS
WITH base AS (
...
agg AS (
  SELECT station,zone,
    ...
    -- Find Death Rate
    CASE
      WHEN SUM(total_crashes) = 0 THEN NULL
      ELSE ROUND(SUM(killed)::FLOAT / SUM(total_crashes), 4)
    END AS death_rate,
    -- Growth Factor
    CASE
      WHEN MIN(total_crashes) = 0 THEN NULL
      ELSE ROUND((MAX(total_crashes) - MIN(total_crashes))::FLOAT / NULLIF(MIN(total_crashes), 0), 2)
...
SELECT
  *,
  last_year AS current_year
FROM agg
WHERE lifetime_crashes > 0;
